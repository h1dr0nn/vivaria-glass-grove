...
/** Submerged substrate reads wet (darker); dry stays warm. */
function fillFor(material: number, isWet: boolean): number {
  if (material === MATERIAL.sand) return isWet ? SCENE.sandWet : SCENE.sand;
  if (material === MATERIAL.soil) return isWet ? SCENE.soilDark : SCENE.soil;
...
export function buildSubstrate(tank: TankState, layout: TankLayout): Container {
  ...
  for (let x = 0; x < tank.width; x++) {
    ...
    drawColumn(g, tank, layout, x, jitter);
  }
...
function drawColumn(
  ...
  // a column reads wet only when it is actually submerged
  const isWet = tank.terrainHeight[x] < tank.waterlineY;
  let runStart = 0;
  ...
  const flush = (end: number): void => {
    ...
    ).fill(shade(fillFor(runMaterial, isWet), jitter));
  };
